...
class App extends React.Component {
  ...
  componentDidMount() {
    this.setState({sunAngle: (12 - this.state.hour) * (360/24)})
     ...
     <div class="bg-drop"  style={{backgroundColor: this.state.secondaryColor}}></div>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 468 468" style={{borderColor: this.state.primaryColor}}>
        <g class="moon" transform={"rotate(" + this.state.moonAngle + " 250 525)"}>
          ...
          <text class="js-text" fill={this.state.primaryColor} fontFamily="MyriadPro-Bold,Myriad Pro" fontSize="115" fontWeight="700" transform="translate(340 450)">{this.state.hour}</text>
        </svg>
        </>
      );
  }
